...
また、**京阪バスの接近情報**や、**Yahoo!路線情報からの運行情報（遅延情報）**の取得もサポートしています。
`httpx` を用いた完全非同期設計で、リアルタイムなアプリケーション開発に適しています。
...
・湖西線: 運転状況
  荒天予想の影響で、一部列車に運休が出ています。16:00頃から、和邇〜近江今津駅間の運転を見合わせます。（12月21日 14時30分掲載）
...
*   `trains: dict[int, TrainData]`: 現在走行中・在線中の列車データ。キーは内部管理番号(WDF)。
...
- **取得できる列車**: API仕様上、現在走行中または準備中のアクティブな列車のみ取得できます。車庫に入っている列車などは取得できません。
